...
import org.jsonrestore.StringOps;

import java.util.Map;

import static org.jsonrestore.Constants.END;
import static org.jsonrestore.Constants.STRING_DELIMITERS;
import static org.jsonrestore.Json.EMPTY_STRING;
import static org.jsonrestore.Json.FACTORY;
import static org.jsonrestore.StringOps.in;
import static org.jsonrestore.StringOps.notIn;
    ...
    public JsonNode parse(JSONParser parser) {
        ...
        while (notIn(ch, END, rstringDelimiter)) {
            ...
            if (!parser.isStreamStable() &&
                    parser.getCurrentContext() == OBJECT_VALUE &&
                    (in(ch, ',', '}')) &&
                    (stringAcc.length() == 0 || StringOps.lastChar(stringAcc) != rstringDelimiter)
            ) {
            ...
            if (!parser.isStreamStable() && ch == ']'
                    ...
                    && StringOps.lastChar(stringAcc) != rstringDelimiter
            ) {
            ...
            if (parser.isStreamStable() && ch != END && StringOps.lastChar(stringAcc) == '\\') {
                stringAcc.setLength(stringAcc.length() - 1);
            }
            if (ch != END && StringOps.lastChar(stringAcc) == '\\') {
                // This is a special case, if people use real strings this might happen
                parser._log("Found a stray escape sequence, normalizing it");
                if (in(ch, rstringDelimiter, 't', 'n', 'r', 'b', '\\')) {
                    StringOps.setLastChar(stringAcc, ESCAPE_SEQS.getOrDefault(ch, ch));
                    parser.shift();
                    ch = parser.getCharAt();
                    while (ch != END && StringOps.lastChar(stringAcc) == '\\' && in(ch, rstringDelimiter, '\\')) {
                        // this is a bit of a special case, if I don't do this it will close the loop or create a train of \\
                        // I don't love it though
                        StringOps.setLastChar(stringAcc, ch);
                        parser.shift();
                        ch = parser.getCharAt();
                    }
                    continue;
                    ...
                    if (nextChars.length() == numChars) {
                        if (nextChars.chars().allMatch(c -> "0123456789abcdefABCDEF".indexOf(c) != -1)) {
                            ...
                            StringOps.setLastChar(stringAcc, (char) Integer.parseInt(nextChars, 16));
            ...
            if (ch == rstringDelimiter && StringOps.lastChar(stringAcc) != '\\') {
        ...
        if (!parser.isStreamStable() && (missingQuotes || StringOps.lastChar(stringAcc) == '\n')) {
